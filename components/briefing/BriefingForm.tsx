...
import { submitBriefingAction } from "@/app/briefing/actions";
import { STEPS, stepsFor, telefoneValido } from "@/app/briefing/fields";
 ...
 *
 * Mora fora da rota porque agora tem duas: `/briefing`, aberto, e
 * `/briefing/<convite>`, que chega com o que já se sabe do cliente. Duplicar
 * as 500 linhas nas duas era o outro caminho.
 */
export function BriefingForm({
  ...
  /** Respostas que já vêm marcadas. Continuam editáveis: quem sabe o nome da
   *  própria empresa é o cliente, não a nossa anotação. */
  prefill?: Record<string, string>;
  /** Amarra a resposta ao convite, pra saber de quem ela veio. */
  linkSlug?: string;
  /** Recado no alto, no lugar da abertura padrão. */
  note?: string;
}) {
  ...
  function advance() {
    ...
    startTransition(async () => {
      const result = await submitBriefingAction(values, linkSlug);
      ...
      <main className="flex min-h-svh items-center justify-center bg-[var(--tatu-cream)] px-4 pt-20 pb-20">
        ...
        >
          <div className="px-6 pt-6 sm:px-8 sm:pt-8">
            ...
            {/* O recado do convite, só no primeiro passo: é uma frase de
                contexto ("como combinamos na conversa de ontem"), não um
                cabeçalho pra acompanhar o cliente até o fim. */}
            {note && step === 0 ? (
              <p className="mt-2 text-sm leading-relaxed text-[var(--tatu-ink)]">
                {note}
              </p>
            ) : null}
          </div>
