...
/**
 * A tela da prospecção — uma só.
 *
 * Eram duas listas dos mesmos contatos (fila por data, tabela por coluna) e
 * um painel de números numa terceira aba. A tabela já mostra a data e já
 * marca o atrasado em vermelho, então a fila só repetia o dado noutra ordem;
 * e número não é lugar, é informação — os três que importam cabem numa linha
 * acima da lista que eles resumem.
 */
export default async function ProspectTablePage({
  ...
  const ganho = wonInMonth(board.prospects, currentMonth());
  ...
  return (
    <div className="mx-auto flex w-full flex-1 max-w-[76rem] flex-col pb-10">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        {/* Os três números em uma linha, acima da lista que eles resumem. Em
            tela própria eles viravam um lugar pra onde ir; aqui são o
            cabeçalho do que já está à vista. */}
        <p className="text-sm text-neutral-600">
          <strong className="font-semibold text-neutral-900 tabular-nums">
            {formatBRL(aberto.value)}
          </strong>{" "}
          em jogo · {formatBRL(ganho.value)} fechado neste mês ·{" "}
          {taxa === null ? "sem decisão ainda" : `${Math.round(taxa * 100)}% de conversão`}
        </p>

        <div className="flex items-center gap-2">
          <ProspectNew stages={board.stages} clients={board.clients} />
          <Link
            href="/admin/prospeccao/ajustes"
            className="inline-flex min-h-10 items-center rounded-md border border-neutral-300 px-3 text-sm text-neutral-600 hover:bg-neutral-50 sm:min-h-0 sm:py-1.5"
          >
            Ajustes
          </Link>
        </div>
        ...
        {FILTERS.map((option) => (
          <Link
            ...
            href={
              option.key === "todos"
                ? "/admin/prospeccao"
                : `/admin/prospeccao?filtro=${option.key}`
            }
